"""Postmortem analysis for a trained two-stage regression gate."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze trained two-stage regression gate")
    ...
    parser.add_argument("--model", required=True, help="Trained two-stage regression gate path")
...
def _summarize_slice(rows: list[dict]) -> dict:
    if not rows:
        return {
            ...
            "avg_stage_a_abs_error": 0.0,
            "avg_stage_b_abs_error": 0.0,
            "oracle_distribution": {},
            "pred_distribution": {},
        }
    stage_b_rows = [r for r in rows if r["intervene_true"]]
    return {
        "count": len(rows),
        ...
        "avg_stage_a_abs_error": round(float(np.mean([r["stage_a_abs_error"] for r in rows])), 4),
        "avg_stage_b_abs_error": round(
            float(np.mean([r["stage_b_abs_error"] for r in stage_b_rows])),
            4,
        ) if stage_b_rows else 0.0,
...
def main():
    ...
    gate.margin_eps = model_config.get("margin_eps", 0.01)
    gate.intervene_margin_threshold = model_config.get("intervene_margin_threshold", 0.0)
    gate.repair_margin_threshold = model_config.get("repair_margin_threshold", 0.0)
    with open(model_path.with_suffix(".pkl"), "rb") as f:
        payload = __import__("pickle").load(f)
    gate.stage_a_model = payload["stage_a_model"]
    gate.stage_b_model = payload["stage_b_model"]
    ...
    for item in all_data:
        ...
        rows.append({
            ...
            "pred_stage_a_margin": float(pred["pred_stage_a_margin"]),
            "pred_stage_b_margin": float(pred["pred_stage_b_margin"]),
            "stage_a_abs_error": abs(float(pred["pred_stage_a_margin"]) - float(target["stage_a_margin"])),
            "stage_b_abs_error": abs(float(pred["pred_stage_b_margin"]) - float(target["stage_b_margin"])),
    ...
    hardest_misses = sorted(
        ...
        key=lambda r: (-r["oracle_margin"], -r["stage_a_abs_error"], r["task"], r["checkpoint_id"]),
    )[: args.top_misses]

    stage_a_sign_errors = [
        r for r in test_rows
        if (r["stage_a_margin"] >= 0) != (r["pred_stage_a_margin"] >= 0)
    ]
    stage_b_sign_errors = [
        r for r in test_rows if r["intervene_true"]
        and (r["stage_b_margin"] >= 0) != (r["pred_stage_b_margin"] >= 0)
    ]
    ...
    result = {
        ...
        "overall": {
            ...
            "avg_stage_a_abs_error": round(float(np.mean([r["stage_a_abs_error"] for r in test_rows])), 4),
            "avg_stage_b_abs_error_on_oracle_intervene": round(
                float(np.mean([r["stage_b_abs_error"] for r in q_or_r_subset])),
                4,
            ) if q_or_r_subset else 0.0,
            "n_stage_a_sign_errors": len(stage_a_sign_errors),
            "n_stage_b_sign_errors": len(stage_b_sign_errors),

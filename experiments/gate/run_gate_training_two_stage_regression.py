"""Train a two-stage margin-regression gate."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train two-stage regression gate")
    ...
    parser.add_argument("--stage-a-min-abs-margin", type=float, default=None)
    parser.add_argument("--intervene-margin-threshold", type=float, default=0.0)
    parser.add_argument("--repair-margin-threshold", type=float, default=0.0)
    ...
    parser.add_argument(
        "--fit-all",
        action="store_true",
        help="Train on all checkpoints without a held-out split",
    )
    parser.add_argument("--run-name", default="gate_train_two_stage_regression_")
...
def main():
    ...
    gate = TwoStageRegressionGate(
        ...
        stage_a_min_abs_margin=args.stage_a_min_abs_margin,
        intervene_margin_threshold=args.intervene_margin_threshold,
